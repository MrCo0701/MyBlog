...
class HomeRepositoryImpl implements HomeRepository {
  ...
  @override
  Future<List<BlogEntity>> showTrendingBlogs() async {
    final url = Env.baseUrl + ApiConstants.postBlog;

    try {
      final res = await dio.get(
        url,
        queryParameters: {"page": 1, "limit": 20, "sort": 'trending'},
      );
  ...
  Future<List<BlogEntity>> searchBlog(String titleSearch) async {
    ...
    try {
      final res = await dio.get(
        url,
        queryParameters: {"page": 1, "limit": 20, "search": titleSearch},
      );
  ...
  @override
  Future<List<BlogEntity>> searchBlogByTag(String tag) async {
    final url = Env.baseUrl + ApiConstants.postBlog;
